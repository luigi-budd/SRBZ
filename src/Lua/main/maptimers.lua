...
--  SRBZ.MapTimers["Your_Timer_Name"].state - is timer active or not, true = is it counting down, false = it is paused
--
--How to actually use it on your map with Linedef Executor (LUA example):
...
--          print("ONE SECOND!!!!")
--      elseif my.timer.time==0 then
--          print("TIME OVER!")
--          dosomethingelse()
--      end
--      if (mytimer.state==false and mytimer.time ~= 0) print("WHY THE FUCK MY TIMER IS STOPPED") end --please ignore this line :skull:
--  end), "FUNCNAME")
...
SRBZ.AddMapTimer=function(n,m,t,a)
	...
	if type(n)!="string" error("Name of the Timer should be string") end
	if type(m)!="number" error("Map ID is not number") end
	if type(t)!="number" error("Time is not number") end
	--if type(timepoint)~="table" and type(timepoint)~="nil" error("TimePoint values must be a table") end
	if type(a)!="boolean" error ("Timer Activity value is not boolean") end
